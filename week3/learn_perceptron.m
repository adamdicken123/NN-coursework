...
for i=1:num_neg_examples
    ...
    if (activation >= 0) % if incorrect update weight
        %YOUR CODE HERE
        w = perceptron_learn(0, x, w);
        fprintf('Updated weight by [%d,%d,%d]\n',w(1)-w_current(1),w(2)-w_current(2),w(3)-w_current(3));
    end
end
for i=1:num_pos_examples
    ...
    if (activation < 0) % if incorrect update weight
        %YOUR CODE HERE
        w = perceptron_learn(1, x, w);
        fprintf('Updated weight by [%d,%d,%d]\n',w(1)-w_current(1),w(2)-w_current(2),w(3)-w_current(3));
    end
end
fprintf('Updated weight by [%d,%d,%d]\n',w(1)-w_current(1),w(2)-w_current(2),w(3)-w_current(3));

function [w_new] = perceptron_learn(target, x, w)
%%
% Implements the perceptron learning procedure to update a weight
%%

% perceptron binary rule
y = 0;
if (x' * w >= 0)
    y = 1;               
end
